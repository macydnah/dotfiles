function fish_prompt
    ...
    # Yazi nested shell indicator
    if set -q YAZI_LEVEL
	# set -f prompt_yazi (set_color $fish_color_user)"| 🗃️ L$YAZI_LEVEL |"$normal" "
	set -f prompt_yazi (set_color blue)"| 🗃️ L$YAZI_LEVEL |"$normal" "
    else
	set -f prompt_yazi ""
    end

    # echo -n -s $prompt_host $cwd $pwd $normal $prompt_status " $delim "
    echo -n -s $prompt_yazi $prompt_host $cwd $pwd $normal $prompt_status " $delim "
end
